...
static const struct snd_kcontrol_new adau1761_left_mixer_controls[] = {
	SOC_DAPM_SINGLE("Left DAC Switch", SND_SOC_NOPM, 0, 1, 0),
	SOC_DAPM_SINGLE("Right DAC Switch", SND_SOC_NOPM, 1, 1, 0),
...
static int adau1761_probe(struct snd_soc_codec *codec)
{
	...
	ret = adau17x1_probe(codec, NULL); // ADAU1761_FIRMWARE
	if (ret < 0)
		return ret;
...
static int adau1761_bus_probe(struct device *dev,
	...
	enum adau17x1_control_type control_type)
{
...
static int adau1761_spi_probe(struct spi_device *spi)
{
	...
	return adau1761_bus_probe(&spi->dev, regmap, type, ADAU17X1_SPI);
}
	...
	const struct i2c_device_id *id)
{
	...
	return adau1761_bus_probe(&client->dev, regmap, type, ADAU17X1_I2C);
}
