...
static void sc16is7x2_handle_rx(struct sc16is7x2_chip *ts, unsigned ch)
{
	...
	written = tty_insert_flip_string(tty, &chan->buf[1], rxlvl);
	if(unlikely(written != rxlvl))
		dev_err(&ts->spi->dev, " unable to write all data");

	/* Update RX counter */
	uart->icount.rx += written;
...
static unsigned int sc16is7x2_get_mctrl(struct uart_port *port)
{
	...
	dev_dbg(&ts->spi->dev, "%s (0x%02x) (0x%02x)\n", __func__,
			chan->msr, chan->mcr);
...
static struct sc16is7x2_platform_data *
sc16is7x2_parse_dt (struct spi_device *spi)
{
	struct device_node		*dn = spi->dev.of_node;
	struct sc16is7x2_platform_data	*pdata;
...
dev_info (&spi->dev, "pdata: gpio_base=%d, uart_base=%d, uartclk=%d, rst_gpio=%d",
	...
	ts = devm_kzalloc(&spi->dev, sizeof(struct sc16is7x2_chip), GFP_KERNEL);
	if (!ts)
		return -ENOMEM;
	...
	if (ret) {
		dev_info (&spi->dev, DRIVER_NAME " Failed to register port 0\n");
		goto exit_destroy;
	}
	ret = sc16is7x2_register_uart_port(ts, pdata, 1);
	if (ret) {
		dev_info (&spi->dev, DRIVER_NAME " Failed to register port 1\n");
		goto exit_uart0;
	}
...
static struct of_device_id	sc16is7x2_of_match[] = {
...
static struct spi_driver sc16is7x2_spi_driver = {
	.driver = {
		...
		.bus		= &spi_bus_type,
		.owner		= THIS_MODULE,
		.of_match_table	= of_match_ptr (sc16is7x2_of_match),
	},
