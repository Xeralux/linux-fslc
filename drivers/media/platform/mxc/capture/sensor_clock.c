...
void set_mclk_rate(uint32_t *p_mclk_freq, uint32_t csi)
{
	...
	} else if (of_machine_is_compatible("fsl,imx6q") || of_machine_is_compatible("fsl,imx6dl")) {
		if (csi == 0) {
			if (of_machine_is_compatible("fsl,imx6q_sabrelite"))
				mclk = "clko2_clk";
			else
				mclk = "clko_clk";
		} else {
	...
	} else if (cpu_is_mx25() || of_machine_is_compatible("fsl,imx6sl")) {	/* only has CSI0 */
		mclk = "csi_clk";
	} else {
		if (csi == 0) {
			mclk = "csi_mclk1";
