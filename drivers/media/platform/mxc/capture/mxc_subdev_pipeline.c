				 ...
				 struct v4l2_frmsizeenum *fsize)
{
	...
	ret = v4l2_subdev_call(input_device, pad, enum_frame_size, NULL, &fse);
	...
	if(ret < 0 && fsize->index < (MODE_TEST_START + subdev_count)) {
		...
		if(fsize->pixel_format != sensor->pix.pixelformat)
			return -EINVAL;
		fse.min_height = fse.max_height = (__u32)sensor->pix.height;
		fse.min_width = fse.max_width = (__u32)sensor->pix.width;
	}
...
static DEVICE_ATTR(operational, 0664, (void *)mxc_pipeline_operational_show, (void *)mxc_pipeline_operational_store);
