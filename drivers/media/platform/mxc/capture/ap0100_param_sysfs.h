				   ...
				   struct device_attribute *attr, const char *buf, int count)
{
	...
	switch (cur_sss_status  ) {
		case SSS_UPDATE :
			ap0100_m034_cmd_write((char*)(buf + sizeof(SENSOR_UPDATE_UUID)), count-sizeof(SENSOR_UPDATE_UUID) );
			printk("received SSS_UPDATE cmd\n");
			break;
		case SSS_READ_TEMPERATURE:
		...
		case SSS_READ_ERROR_CNT:
			...
			break;
		case SSS_IDLE:
				printk("sensor sysfs command error!\n");
			break;
	}
